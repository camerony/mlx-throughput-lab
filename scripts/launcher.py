#!/usr/bin/env python3
"""Simple text-based launcher for MLX throughput tests (no dialog dependency)."""
import os
import shlex
import subprocess
import sys
...
def build_commands(python_bin: str) -> Dict[str, Tuple[str, List[str]]]:
    return {
        "1": (
            "Single request test",
            [python_bin, "-m", "unittest", "tests/test_mlx_server_single.py"],
        ),
        "2": (
            "Concurrent requests test",
            [python_bin, "-m", "unittest", "tests/test_mlx_server_concurrent.py"],
        ),
        "3": (
            "Round-robin test (nginx + multiple servers)",
            [python_bin, "-m", "unittest", "tests/test_mlx_server_round_robin.py"],
        ),
        "4": (
            "Concurrency sweep test",
            [python_bin, "tests/test_mlx_server_concurrency_sweep.py"],
        ),
        "5": (
        ...
        "6": (
            "Full sweep (instances x concurrency)",
...
def main() -> int:
    ...
    while True:
        ...
        raw_overrides = input(
            "Optional env overrides (e.g. MLX_CONCURRENCY=64 "
            "MLX_NUM_REQUESTS=64), or Enter to use defaults: "
        ).strip()
